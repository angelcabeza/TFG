...
def need_preparation(method, folder_path):
    num_csv_files = len(glob.glob1(folder_path,"*.csv"))
    
    folder_preparation = True
    
    if (num_csv_files == 15):
        print("Saltando la preparación y conversión de los landmarks ground truth\
              del método DECA")
        folder_preparation = False
    elif (num_csv_files != 0):
        print("Borrando los archivos csv existentes para iniciar la preparación\
              de los landmarks desde 0 del método DECA")
    
        files = os.listdir('./lmk_truth_deca')
        for item in files:
            if item.endswith(".csv"):
                os.remove(os.path.join('./lmk_truth_deca', item))
...
if __name__ == '__main__':
    ...
    folder_preparation_deca = need_preparation('DECA', './lmk_truth_deca')
    
    if folder_preparation_deca:
        folder_preparation("./lmk_truth_deca", "./predicted_lmks/DECA")       
        
    for file in os.listdir('./predicted_lmks/DECA'):
        if file.endswith('.pp'):
            gt_lmk_path = './lmk_truth_deca' + '/' + file
            gt_lmks_dict = filter_gt_landmarks.get_predicted_landmarks(gt_lmk_path)
            
            created_file = './predicted_lmks/DECA' + '/' + 'filtered_' + file
            
            print("created_file: ", created_file)
            filter_gt_landmarks.filter_pp_file(gt_lmks_dict, './predicted_lmks/DECA' + '/' + file, created_file)
            
    for file in os.listdir('./predicted_lmks/DECA'):
        if "filtered" in file:
            filter_gt_landmarks.pp_to_csv('./predicted_lmks/DECA' + '/' + file)
        
    
    sys.exit("Todo bien maquina")
    ...
    GT_df_VRN = read_GT_PP_from_csv(dic, "/home/angel/Escritorio/NowDatasetExp2/lmk_truth_vrn")
    PREDICTED_df_VRN = read_predicted_lmk_from_csv(dic, "/home/angel/Escritorio/EvalDataset2/VRN/landmarksFiltered", GT_df_VRN)
    GT_df_DECA = read_GT_PP_from_csv(dic, "/home/angel/Escritorio/NowDatasetExp2/lmk_truth")
    PREDICTED_df_DECA = read_predicted_lmk_from_csv(dic, "/home/angel/Escritorio/EvalDataset2/DECA/landmarksFiltered", GT_df_DECA)
    GT_df_P2V = read_GT_PP_from_csv(dic, "/home/angel/Escritorio/NowDatasetExp2/lmk_truth_p2v")
    PREDICTED_df_P2V = read_predicted_lmk_from_csv(dic, "/home/angel/Escritorio/EvalDataset2/p2v/landmarksFiltered", GT_df_P2V)
    GT_df_MVFNET = read_GT_PP_from_csv(dic, "/home/angel/Escritorio/NowDatasetExp2/lmk_truth_mvfnet")
    PREDICTED_df_MVFNET = read_predicted_lmk_from_csv(dic, "/home/angel/Escritorio/EvalDataset2/mvfnet/landmarksFiltered", GT_df_MVFNET)
    ...
    #print("Estadísticas de error de DECA: ", est_error_deca.to_string())
    
    
    #est_error_deca.to_csv('./est_error_DECA.csv', sep='\t')
    #est_error_deca.to_excel('./est_error_VRN.xlsx')
    
    #print("Estadísticas de error por Landkmark de DECA: ", est_lmk_deca.to_string())
    
    #est_lmk_deca.to_excel('./est_lmk_VRN.xlsx')
    
    #print("Estadísticos DECA\n: ", tmp.to_string())
    
    #tmp.to_csv('./est_generales_DECA', sep='\t')
    
    #error_df.to_excel('./error_exp2.xlsx')
    #error_df.to_csv('./error_exp2.csv', sep='\t')
# =============================================================================
